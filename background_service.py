...
def set_wallpaper():
    ...
    if connected == "1" and (prevConnected == "2" or prevImg1 != img1):
        ...
        if (
            prevImg1 != img1
            or prevImg2 != img2
            or prevConnected != connected
            or prevIsImg1Left != f"{isImg1Left}"
        ):
            previousSettings[0][1] = img1
            ...
            run_subprocess_command(
                f"convert {f'{img1} {img2}' if isImg1Left else f'{img2} {img1}'} -resize 1920x1080 -background black -gravity center -extent 1920x1080 +append {imgActive}"
            )
